            ...
            <table class="table table-hover">
                ...
                @foreach ($students as $student)
                    <tr>
                        ...
                        <td>
                            <button data-url="{{ route('student.show',$student->id) }}"​ type="button" data-target="#show" data-toggle="modal" class="btn btn-info btn-show">Detail</button>
                            <button data-url="{{ route('student.update',$student->id) }}"​ type="button" data-target="#edit" data-toggle="modal" class="btn btn-warning btn-edit">Edit</button>
                            <button data-url="{{ route('student.destroy',$student->id) }}"​ type="button" data-target="#delete" data-toggle="modal" class="btn btn-danger btn-delete">Delete</button>
                        </td>
                    </tr>
                @endforeach
                </tbody>
            ...
            $('.btn-show').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('h3#id').text(response.data.id)
                        $('h3#hoten').text(response.data.hoten)
                        $('h3#gioitinh').text(response.data.gioitinh)
                        $('h3#ngaysinh').text(response.data.ngaysinh)
                        $('h3#sdt').text(response.data.sdt)
                        $('h3#diachi').text(response.data.diachi)
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        //xử lý lỗi tại đây
                    }
            ...
            $('.btn-edit').click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('#form-edit').attr('data-url', '{{ asset('student/') }}/' + response.data.id)
                    },
